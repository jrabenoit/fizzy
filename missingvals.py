...
def Impute():
    '''Imputes missing vals by mean, scales as sparse matrix, 96% sparse after scaling'''
    ...
    #str((np.count_nonzero(X)/(X.shape[0]*X.shape[1]))*100) to get density... about 4% atm
    
    
    #Should return to the psych scales (madrs, hamd etc) and encode using OneHotEncoder, since continuous variables are expected.
    
    np.savetxt('/media/james/ext4data1/current/projects/pfizer/ready.csv', X, delimiter=',')
