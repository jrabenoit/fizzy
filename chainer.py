...
def InnerFolds():
    ...
    est= {'randomforest': ensemble.RandomForestClassifier(), 
          ...
          'neuralnet': neural_network.MLPClassifier(solver='sgd', max_iter=10000)
          'voting': ensemble.VotingClassifier(estimators=[('rf', ensemble.RandomForestClassifier()),('kn', neighbors.KNeighborsClassifier()),('nn', neural_network.MLPClassifier(solver='sgd', max_iter=10000))])
          }
    ...
    for i in range(folds):
        ...
        for j,k in zip(est.keys(), est.values()):
            X_train=k.fit_transform(X_train, y_train)
